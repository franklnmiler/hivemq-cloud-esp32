...
def init_sensor_routes(app, mysql, socketio, firebase_ref, should_use_firebase_only, mirror_mysql_to_firebase, send_data):
    ...
    # =====================================
    # API CHART SUHU SAJA (TIDAK DIGUNAKAN LAGI)
    # =====================================
    # @sensor_bp.route('/api/bme280/chart')
    # def chart_data():
    #     # ... (kode sebelumnya) ...
    ...
    # =====================================
    # API CHART MULTI PARAMETER (5 MENIT TERAKHIR)
    # =====================================
    @sensor_bp.route('/api/bme280/chart-multi')
    def chart_data_multi():
        try:
            if should_use_firebase_only():
                data = firebase_ref.child('sensor_data').order_by_child("waktu").get()
                if not data:
                    return jsonify({
                        "labels": [],
                        "suhu": [],
                ...
                data_filtered = [
                    ...
                    if datetime.strptime(d["waktu"], "%Y-%m-%d %H:%M:%S") >= datetime.now() - timedelta(minutes=5)
                ]
                waktu = [datetime.strptime(d["waktu"], "%Y-%m-%d %H:%M:%S").strftime('%H:%M:%S') for d in data_filtered]
                suhu = [d.get("suhu") for d in data_filtered]
                kelembapan = [d.get("kelembapan") for d in data_filtered]
                tekanan = [d.get("tekanan") for d in data_filtered]
                altitude = [d.get("altitude") for d in data_filtered]
                mq135_ppm = [d.get("mq135_ppm") for d in data_filtered]
                jarak = [d.get("jarak_ultrasonik") for d in data_filtered]
            else:
                cursor = mysql.connection.cursor()
                cursor.execute("""
